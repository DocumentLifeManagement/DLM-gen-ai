...
export default function UploaderDashboard({ navigate }) {
  ...
  const handleFilesSelected = (files) => {
    ...
    const validFiles = Array.from(files).filter((file) => {
      ...
      if (!isValid) {
        showToast(
          `Skipped ${file.name}: Unsupported format. Only PDF, JPEG, PNG, and TIFF are allowed.`,
        );
      }
      return isValid;
    });
  ...
  const filteredDocs = documents.filter((d) => {
    ...
    if (activeTab === "rejected") {
      return (
        matchesSearch && (d.status === "REJECTED" || d.status === "FAILED")
      );
    }
    ...
    <DashboardLayout role={userRole} navigate={navigate} title="Upload Center">
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <p className="text-slate-400 mb-6">
              Support for PDF, JPEG, PNG, and TIFF up to 500MB
            </p>
            <div className="flex gap-4 justify-center text-xs text-slate-500 font-mono">
              <span className="bg-brand-900/50 px-3 py-1 rounded border border-brand-800">
                PDF
              </span>
              <span className="bg-brand-900/50 px-3 py-1 rounded border border-brand-800">
                IMAGE
          ...
          <AnimatePresence>
            {uploadQueue.length > 0 && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 20 }}
                className="bg-brand-900 border border-brand-800 rounded-2xl overflow-hidden shadow-2xl"
              >
                ...
                <div className="p-4 space-y-3">
                  {uploadQueue.map((item) => (
                    <motion.div
                      ...
                      className="bg-brand-950/50 border border-brand-800 rounded-xl p-4"
                    >
                          ...
                          <div className="flex justify-between items-center text-[10px] font-mono mb-2">
                            ...
                            <div className="flex gap-4 items-center">
                              {item.status === "uploading" && (
                                <span className="text-brand-accent animate-pulse font-black italic">
                                  Est.{" "}
                                  {Math.max(
                                    2,
                                    Math.ceil(item.file.size / 800000),
                                  )}
                                  s remaining
                                </span>
                              )}
                              <span
                                className={`font-black uppercase tracking-widest ${item.status === "done" ? "text-emerald-500" : item.status === "error" ? "text-rose-500" : "text-brand-accent"}`}
                              >
                                {item.status === "pending"
                                  ? "Standby"
                                  : item.status === "uploading"
            ...
            <div className="flex items-center gap-6 border-b border-brand-800 w-full md:w-auto">
              ...
              >
                ...
                {activeTab === "rejected" && (
                  <motion.div
                    layoutId="uploaderTab"
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.5)]"
                  />
                )}
              </button>
            </div>
            ...
            >
              {paginatedDocs.map((doc) => (
                ...
                >
                      ...
                      <div
                        className={clsx(
                          ...
                          selectedIds.includes(doc.id)
                            ...
                            : "border-brand-800 bg-brand-950 group-hover:border-brand-600",
                        )}
                      >
                        {selectedIds.includes(doc.id) && (
                  ...
                  >
                    ...
                    >
                      <button
                        ...
                        className="p-2 bg-brand-800 rounded-lg hover:text-brand-accent transition-colors"
                      >
                        <Eye size={14} />
                      </button>
                    </div>
                  ...
                  <p
                    className="text-sm font-bold text-white mb-1 truncate pr-16"
                    title={doc.filename}
                  >
                    {doc.filename}
                  </p>
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <span
                      className={`text-[9px] font-black px-2 py-0.5 rounded border tracking-widest uppercase ${statusBadge(doc.status)}`}
                    >
      ...
      <AnimatePresence>
        {previewDoc && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-end md:items-center justify-center z-50 p-0 md:p-6"
          ...
          >
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="bg-brand-900 border border-brand-800 rounded-t-3xl md:rounded-3xl w-full max-w-6xl h-[92vh] md:h-[85vh] overflow-hidden flex flex-col shadow-2xl"
            >
              <div className="px-8 py-6 border-b border-brand-800 flex justify-between items-center bg-brand-950">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-brand-800 flex items-center justify-center text-brand-accent shadow-inner">
                    <FileText size={24} />
                  </div>
                  <div>
                ...
                <button
                  onClick={() => setPreviewDoc(null)}
                  className="p-2 hover:bg-brand-800 rounded-full transition-colors text-slate-500 hover:text-white"
                >
                  <XCircle size={28} />
                </button>
              </div>
                ...
                <div className="flex-1 space-y-4 md:space-y-6 overflow-y-auto custom-scrollbar max-h-[40vh] lg:max-h-none">
                  <div className="p-6 bg-brand-accent/5 border border-brand-accent/10 rounded-2xl">
                    <h4 className="text-[10px] font-black text-brand-accent uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                      <ShieldCheck size={14} /> System Metadata
                    </h4>
              ...
              <div className="p-8 bg-brand-950 border-t border-brand-800 flex justify-end">
                <Button
                  variant="primary"
                  onClick={() => setPreviewDoc(null)}
                  className="!px-12 !py-4 font-black uppercase text-xs tracking-[0.2em]"
                >
                  Close Preview
                </Button>
              </div>
      ...
      {toast && (
        <motion.div
          ...
          className="fixed bottom-10 right-10 bg-brand-accent text-white px-8 py-5 rounded-2xl shadow-2xl z-50 font-black uppercase tracking-widest text-xs flex items-center gap-4"
        >
