              ...
              <div className="flex flex-wrap items-center gap-2 md:gap-6">
                <div className="flex items-center gap-3">
                  <div className="w-20 bg-brand-800 h-1.5 rounded-full overflow-hidden">
                    <div
                      className="bg-brand-cyan h-full shadow-[0_0_8px_rgba(6,182,212,0.5)]"
                      style={{ width: `${avgConfidence}%` }}
                    />
                  </div>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                    {avgConfidence}% Integrity
                  </span>
                </div>
          ...
          <div className="bg-brand-900 border border-brand-800 rounded-2xl flex flex-col shadow-2xl overflow-hidden group">
            <div className="px-5 py-3 border-b border-brand-800 flex justify-between items-center bg-brand-950/40">
              <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-2">
                <Database size={12} className="text-brand-cyan" /> Secure Object
                View
              </span>
              <button
            ...
            <div className="flex-1 bg-brand-950/80 p-2 relative">
              ...
              <div className="absolute bottom-6 right-6 pointer-events-none p-3 bg-brand-950/90 border border-brand-800 rounded shadow-2xl backdrop-blur-md">
                <p className="text-[8px] font-mono text-brand-cyan uppercase">
                  System UID: {id}
                </p>
                <p className="text-[8px] font-mono text-slate-600 mt-1 uppercase">
                  S3 Signature: Verified
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex border-b border-brand-800 bg-brand-950/40">
              ...
              >
                ...
                {activeTab === "metadata" && (
                  <motion.div
                    layoutId="tab"
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-brand-cyan shadow-[0_0_10px_rgba(6,182,212,0.8)]"
                  />
                )}
              </button>
              <button
                ...
                {activeTab === "metadata" ? (
                  ...
                  >
                    {fields.map((field, idx) => (
                      <div
                        key={idx}
                        className="bg-brand-950/20 p-4 rounded-2xl border border-brand-800/50 space-y-3 hover:border-brand-cyan/30 transition-all group"
                      >
                        <div className="flex justify-between items-center">
                          <label className="text-[10px] text-brand-cyan font-black uppercase tracking-widest">
                            {field.key}
                          </label>
                          <span className="text-[9px] text-slate-600 font-mono bg-brand-900 px-1.5 py-0.5 rounded">
                            ...
                            <div className="p-4 bg-brand-950/50 border border-brand-800 rounded-xl">
                              ...
                              <div className="mt-2 flex flex-col gap-0.5">
                                <p className="text-[10px] text-slate-400">
                                  <span className="text-slate-500 uppercase text-[8px] font-black tracking-widest mr-1">
                                    {entry.actor_type}
                                  </span>
                        ...
                        {!lifecycle.length && (
                          <div className="relative">
                            ...
                            <div className="p-4 bg-brand-950/50 border border-brand-800 rounded-xl">
                              ...
                              <p className="text-[10px] text-slate-400">
                                Record successfully vaulted into secure storage.
                              </p>
                            </div>
                          </div>
                        )}
          ...
          <div className="flex-1 space-y-3">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                <Zap size={10} className="text-brand-cyan" /> Final Authority
                Note
              </label>
              <span className="text-[9px] text-slate-600 font-mono tracking-tighter uppercase font-black">
                Admin Override Buffer Enabled
              </span>
            </div>
            <textarea
              value={notes}
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-xs font-black text-white uppercase tracking-tight">
                    {documentData.status === "ARCHIVED"
                      ? "Master Seal Applied"
                      : "Root Identity Attestation"}
                  </h4>
                  <p className="text-[10px] text-slate-500 font-medium leading-tight italic">
                    {documentData.status === "ARCHIVED"
                      ? "Contract execution finalized and vaulted."
                      : "Applying master administrative seal."}
                  </p>
                </div>
              </div>
              <div
                className={clsx(
                  ...
                  documentData.status === "ARCHIVED" || digitallySigned
                    ? "bg-brand-cyan border-brand-cyan text-brand-950"
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
            ...
            >
              ...
              <p className="text-sm text-slate-400 mb-10 leading-relaxed max-w-xs mx-auto">
                Warning: This document and all associated metadata will be
                permanently erased from S3 and the SQL database. This is{" "}
        ...
        >
          <div
            className={clsx(
              ...
              toast.type === "error"
                ? "bg-red-500"
                : "bg-brand-cyan shadow-[0_0_8px_rgba(6,182,212,1)]",
            )}
          />
