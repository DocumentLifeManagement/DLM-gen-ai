...
export default function UserManagement({ navigate }) {
    ...
    const handleDeleteUser = async (userId, userEmail) => {
        if (!window.confirm(`Are you sure you want to terminate authorization for ${userEmail}?`)) {
            return;
        }
    ...
    if (loading) {
        return (
            <DashboardLayout role={userRole} navigate={navigate} title="User Administration">
                <div className="flex flex-col items-center justify-center h-64">
                    <div className="w-8 h-8 border-2 border-brand-accent border-t-transparent rounded-full animate-spin mb-4" />
                    <p className="text-slate-500 text-sm">Loading security directory...</p>
                </div>
            </DashboardLayout>
        );
    }
